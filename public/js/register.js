    ...
    if (arr_login && arr_password && arr_confirm_password){
        ...
        if (typeof login == "string" && typeof password == "string" && typeof confirm_password == "string"){
            if (password == confirm_password){
                $.ajax({
                    url: "http://nyte.fr:8080/api/register",
                    type: "POST",
                    data: {
                        login: login,
                        password: password
                    ...
                    success: (res)=>{
                        display_register_info("success", "Creation du compte réussi avec succès");
            ...
            display_register_info("error", "Une erreur est survenu dans le formulaire");
        }
    }else{
        display_register_info("error", "Une erreur est survenu dans le formulaire");
    }
