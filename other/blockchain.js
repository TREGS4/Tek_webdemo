...
class BLOCKCHAIN_DATA {
    ...
    ask_server() {
        request('http://node.tek.ollopa.fr:7072/server/count', { json: true }, (err, res, body) => {
            if (err) {
    ...
    ask_blockchain() {
        request('http://node.tek.ollopa.fr:7072/blockchain', { json: true }, (err, res, body) => {
            if (err) {
                let error = err.toString();
                //console.log(error);
                if (error.includes("Error: connect ECONNREFUSED")) { // blockchain offline
    ...
    ask_tl() {
        request('http://node.tek.ollopa.fr:7072/transactions/get', { json: true }, (err, res, body) => {
            if (err) {
                let error = err.toString();
                //console.log(error);
                if (error.includes("Error: connect ECONNREFUSED")) { // blockchain offline
